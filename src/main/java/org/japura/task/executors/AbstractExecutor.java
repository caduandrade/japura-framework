...
public abstract class AbstractExecutor implements Executor{
  ...
  public AbstractExecutor(TaskExecutor taskExecutor) {
	if (taskExecutor == null) {
	  throw new TaskExeception(TaskExecutor.class.getSimpleName()
		  + " can't be NULL");
	}
  ...
  protected void fireSubmitEvents(TaskSession session, Task[] tasks) {
	for (Task task : tasks) {
	  try {
		task.submitted(session);
	  } catch (Exception e) {
